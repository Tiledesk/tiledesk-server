    ...
    // async send(sender, senderFullname, recipient, text, id_project, createdBy, attributes, type, metadata, language) {
      ...
      try {
        ...
        if (sender === "system") {
            ...
            let id = sender.replace("bot_","");     // botprefix
            winston.debug("bot id: "+id);
            sender = id; //change sender removing bot_
            let bot = await Faq_kb.findById(id)    //TODO add cache_bot_here non sembra scattare.. dove viene usato?
            ...
            let user = await User.findById(sender)                                //TODO user_cache_here
...
let sendMessageUtil = new SendMessageUtil();
