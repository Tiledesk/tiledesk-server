...
router.get('/', function (req, res) {
  ...
  var skip = page * limit;
  console.log('Activity ROUTE - SKIP PAGE ', skip);
  // winston.debug('Activity ROUTE - SKIP PAGE ', skip);

  console.log('Activity ROUTE - QUERY ', req.query)
  ...
  if (req.query.start_date && req.query.end_date) {
    console.log('Activity ROUTE - REQ QUERY start_date ', req.query.start_date);
    console.log('Activity ROUTE - REQ QUERY end_date ', req.query.end_date);
    ...
    console.log('Activity ROUTE - REQ QUERY FORMATTED START DATE ', startDate);
    console.log('Activity ROUTE - REQ QUERY FORMATTED END DATE ', endDate);

    // ADD ONE DAY TO THE END DAY
    var date = new Date(endDate);
    var newdate = new Date(date);
    var endDate_plusOneDay = newdate.setDate(newdate.getDate() + 1);
    console.log('Activity ROUTE - REQ QUERY FORMATTED END DATE + 1 DAY ', endDate_plusOneDay);
    ...
    console.log('Activity ROUTE - QUERY CREATED AT ', query.createdAt);

  } else if (req.query.start_date && !req.query.end_date) {
    console.log('Activity ROUTE - REQ QUERY END DATE IS EMPTY (so search only for start date)');
    var startDate = moment(req.query.start_date, 'DD/MM/YYYY').format('YYYY-MM-DD');

    query.createdAt = { $gte: new Date(Date.parse(startDate)).toISOString() };
    console.log('Activity ROUTE - QUERY CREATED AT (only for start date)', query.createdAt);
  }

  if (req.query.agent_id) {
    console.log('req.query.agent', req.query.agent_id);
    query["$or"] = [
  ...
  if (req.query.activities) {
    console.log('req.query.activities:', req.query.activities);
  ...
  console.log('Activity ROUTE - Activity.find(query) ', query)
  return Activity.find(query).
    ...
    exec(function (err, activities) {
      ...
      return Activity.count(query, function (err, totalRowCount) {
        ...
        console.log('Activity ROUTE - objectToReturn ', objectToReturn);

        objectToReturn.activities.forEach(activity => {
          console.log('Activity ROUTE - activity.target ', activity.target);
          if (activity.target && activity.target.object && activity.target.object.id_user) {
            console.log('Activity ROUTE - *** 9-+activity.target.id_user ', activity.target.object.id_user._id);
          }
        });
...
router.get('/csv', function (req, res) {
  ...
  return Activity.find(query).
    ...
    exec(function (err, activities) {
      ...
      console.log('csvActivitiesToReturn: ', csvActivitiesToReturn);
    ...
    if (activity.actor && activity.actor.id) {
      if (activity.target &&
        ...
        if (activity.actor.id === target_user_id) {
          ...
          console.log('here + activity.actionObj.user_available ', activity.actionObj.user_available)
  ...
  function buildCsv(activities, lang) {
    ...
    activities.forEach(activity => {
      ...
      // if (lang) {
      console.log('buildCsv lang: ', lang);
