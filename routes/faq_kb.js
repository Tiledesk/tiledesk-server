...
router.put('/:faq_kbid/publish', roleChecker.hasRole('admin'), async (req, res) => {
  ...
  try {
    ...
    let updatedForkedChabot = await Faq_kb.findByIdAndUpdate(forkedChatBotId, { $unset: { slug: 1 }, $set: { trashed: true, original_id: id_faq_kb, publishedBy: req.user.id, publishedAt: new Date().getTime() } }, { new: true, upsert: true }).exec();
