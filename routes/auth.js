...
router.post('/signinWithCustomToken', [
  ...
  let role = req.user.role || RoleConstants.USER;

  // Variabili necessarie in più blocchi
  let newUser;
  let project_userUser;
  let signOptions;
  let alg;
  ...
  if (AudienceType === "subscriptions") {
    ...
    const subscription = await Subscription.findById(AudienceId).exec();
  ...
  Project_user.findOne({ id_project: id_project, uuid_user: req.user._id })
    .exec(async (err, project_user) => {
      ...
      if (!project_user) {
        ...
        if ([RoleConstants.OWNER, RoleConstants.ADMIN, RoleConstants.AGENT].includes(role)) {
          createNewUser = true;
          winston.debug('role owner or admin or agent');

          try {
            newUser = await userService.signup(
            ...
            if (e.code === "E11000") {
              ...
              project_userUser = await Project_user.findOne({ id_project: id_project, id_user: newUser._id }).exec();
              if (project_userUser && project_userUser.status === "active") {
                signOptions = {
                  issuer: 'https://tiledesk.com',
                  subject: 'user',
                  audience: 'https://tiledesk.com',
                ...
                alg = process.env.GLOBAL_SECRET_ALGORITHM;
                if (alg) signOptions.algorithm = alg;

                let userJson = newUser.toObject();
                delete userJson.password;

                let returnToken = jwt.sign(userJson, configSecret, signOptions);
                if (!returnToken.startsWith("JWT")) returnToken = "JWT " + returnToken;
        ...
        if (createNewUser) newProject_user.id_user = newUser._id;
        ...
        return newProject_user.save(function (err, savedProject_user) {
          ...
          if (createNewUser) {
            ...
            returnToken = jwt.sign(userJson, configSecret, signOptions);
          }
        ...
        if (project_user.status === "active") {
          ...
          if ([RoleConstants.OWNER, RoleConstants.ADMIN, RoleConstants.AGENT].includes(req.user.role)) {
            const userFromDB = await User.findOne({ email: req.user.email.toLowerCase(), status: 100 }).exec();
