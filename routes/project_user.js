...
router.post('/invite', [passport.authenticate(['basic', 'jwt'], { session: false }), validtoken, roleChecker.hasRole('admin')], function (req, res) {
  ...
  User.findOne({ email: email, status: 100 }, (err, user) => {
    ...
    if (!user) {
      ...
      Project_user.find({ id_project: id_project, id_user: user._id, role: { $in: roles }}, (err, puser) => {
        if (err) {
        ...
        if (puser) {
          ...
          Project_user.findByIdAndUpdate(puser._id, { role: req.body.role, user_available: user_available, trashed: false, status: 'active' }, (err, updatedPuser) => {
            if (err) {
