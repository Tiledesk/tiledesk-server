...
async function up () {
  await new Promise((resolve, reject) => {
    ...
    // return mongoose.connection.db.collection('triggers').updateOne({"conditions.all.path":"availableAgentsCount"},{ "$set": { "conditions.all.$.path": "snapshot.availableAgentsCount" } }, function (err, updates) {
      return mongoose.connection.db.collection('triggers').find({"conditions.all.path":"availableAgentsCount"}).toArray( function (err, triggers) {
        if (err) {
        ...
        triggers.forEach( 
          function(trigger, i){ 
            ...
            return mongoose.connection.db.collection('triggers').updateOne({_id:trigger._id, "conditions.all.path":"availableAgentsCount"},{ "$set": { "conditions.all.$.path": "snapshot.availableAgentsCount" } }
            // {
            //   multi: true,
      ...
      winston.info("Schema updated for triggers "+triggers.length);
       return resolve('ok'); 
    });  
